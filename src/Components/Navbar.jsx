import React, { useState } from "react";

function Navbar() {
    const [isOpen, setIsOpen] = useState(false);
  return (
    <div>
     
      <nav class="bg-white text-black fixed w-full head  absoulute   z-20 top-0 start-0    ">
        <div
        className={`fixed top-0 left-0  h-full w-0 overflow-x-hidden bg-black text-white pt-16 transition-all duration-500 ease-in-out ${
          isOpen ? 'w-[450px]' : 'w-0'
        }`}
      >
        <button
          onClick={() => setIsOpen(false)}
          className="absolute top-0 right-6 text-4xl focus:outline-none"
        >
          &times;
        </button>
        <nav className="flex flex-col space-y-4 pl-8 mt-4 text-xl">
          <a href="#" className="text-gray-400 hover:text-white transition">
            About
          </a>
          <a href="#" className="text-gray-400 hover:text-white transition">
            Services
          </a>
          <a href="#" className="text-gray-400 hover:text-white transition">
            Clients
          </a>
          <a href="#" className="text-gray-400 hover:text-white transition">
            Contact
          </a>
        </nav>
      </div>

      {/* Main Content */}
     
        <div class="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
          <div className="flex gap-2">

           
          <a
            href="https://flowbite.com/"
            class="flex items-center space-x-3 rtl:space-x-reverse w-16"
          >
            
            <svg
              xmlns="http://www.w3.org/2000/svg"
              viewBox="10.63 8.863 708.662 467.493"
              >
              <g transform="translate(-4386.46913,-568.248104)" fill="#CC071E">
                <path d="M4895.119,591.292c25.74-12.642,38.496-10.983,38.783,1.831 c0.344,16.646-2.117,38.782-3.891,54.799c-9.553,87.055-25.512,158.191-26.713,247.574 c42.043-108.843,77.279-184.492,122.697-277.662c14.416-29.688,23.625-24.139,35.465-29.402 c46.105-20.535,47.822-7.951,41.814,17.161c-22.365,93.01-79.625,385.858-88.375,430.951c-2.518,13.047-16.703,7.516-20.365,2.414 c-16.416-22.709-34.949-23.064-32.891-38.165c10.297-75.181,47.363-263.808,57.031-308.254 c-49.252,100.972-100.332,227.199-126.588,298.77c-5.549,15.198-15.674,14.083-21.965,2.963 c-8.867-15.621-26.084-23.596-28.945-42.055c-8.98-58.688,10.297-170.552,12.984-241.322 c-26.826,77.308-71.844,227.399-91.579,295.211c-8.122,28.058-35.236,23.481-28.029-3.866 c29.973-113.803,94.555-315.078,122.469-387.438C4863.602,597.755,4880.818,598.327,4895.119,591.292" />
                <path d="M4731.637,577.334c-8.694-2.116-33.92,11.498-49.422,14.186 c-4.919,0.858-9.61,6.063-11.269,9.782c-24.826,57.03-47.077,111.394-66.354,160.919c-24.654,4.21-53.483,9.753-85.745,17 c23.109-59.278,46.448-117.744,69.214-173.973c11.211-27.686-18.304-30.317-29.745-2.288 c-14.873,36.438-42.73,104.708-73.562,184.281c-22.652,5.554-46.676,11.903-71.788,19.157 c-18.248,5.268-18.991,10.13-11.097,21.073c4.347,6.093,13.328,5.479,17.446,9.821c10.64,11.252,17.104,24.711,36.151,26.748 c-16.989,45.446-33.978,92.312-49.365,137.438c-9.667,28.326,17.847,34.12,28.43,4.146c17.046-48.318,35.007-97.22,53.483-145.978 c15.387-3.576,52.625-11.596,87.003-19.055c-27.285,74.785-45.247,131.581-52.053,159.227c-1.258,5.319,0.858,8.271,2.002,10.439 c9.209,13.402,17.961,13.951,29.802,30.522c3.204,4.519,13.958,6.956,17.618-4.17c25.283-76.37,51.31-148.026,75.563-212.264 c10.067-2.202,28.2-6.578,39.297-23.138c19.734-29.391,25.397-23.43,30.146-32.365c5.949-11.217,2.001-21.748-19.048-19.146 c0,0-7.951,0.566-22.652,2.242c23.338-60.09,43.759-111.057,58.46-149.896C4739.188,588.889,4739.817,579.279,4731.637,577.334" />
                <path d="M4697.202,967.071c5.434,13.734,22.423,9.575,15.959-7.001 c-6.579-16.887-22.938-67.722-27.229-84.099c-5.834-22.068,19.449-16.033,6.636-1.606c-10.982,12.406-18.305,17.377-38.611,40.967 c-19.62,22.772-14.3,52.323,6.693,58.85c23.681,7.367,44.56-18.728,57.487-37.873c12.527-18.539,0.4-29.413-12.241-18.453 c-8.123,7.047-17.504,17.869-27,23.704c-7.722,4.696-13.786-0.349-3.489-14.523c11.955-16.411,25.34-30.162,35.293-45.67 c20.307-31.604-19.849-49.822-37.124-29.293c-9.267,10.943-6.921,21.686-4.404,30.906 C4673.234,897.749,4688.335,944.895,4697.202,967.071" />
              </g>
            </svg>
          </a>
          <div className="p-6">
           
         
        <span
          className="text-3xl cursor-pointer"
          onClick={() => setIsOpen(true)}
          >
          <div class="space-y-1">
  <div class="w-5 h-0.5 bg-black"></div>
   <div class="w-5 h-0.5 bg-black"></div>
</div>
        </span>
      </div>
      <div
            class="items-left justify-between hidden w-full md:flex md:w-auto md:order-1"
            id="navbar-sticky"
          >
       <ul class="flex flex-col p-4 md:p-0 mt-4 text-black  font-medium border     md:space-x-8 rtl:space-x-reverse md:flex-row  md:border-0     ">
              <li>
                <a
                  href="#"
                  class="block py-2 px-3 text-black    rounded-sm md:bg-transparent  md:p-0 "
                  aria-current="page"
                >
                  Home
                </a>
              </li>
              <li>
                <a
                  href="#"
                  class="block py-2 px-3 text-black  rounded-sm  md:hover:bg-transparent   md:p-0 "
                >
                  Men
                </a>
              </li>
              <li>
                <a
                  href="#"
                  class="block py-2 px-3 text-black  rounded-sm  md:hover:bg-transparent   md:p-0 "
                >
                  Ladies
                </a>
              </li>
              <li>
                <a
                  href="#"
                  class="block py-2 px-3 text-black  rounded-sm  md:hover:bg-transparent   md:p-0 "
                >
                  Kids
                </a>
              </li>
            </ul>
            </div>
              </div>
          <div class="flex md:order-2 space-x-2 md:space-x-0 gap-5">

            {/* 1st logo */}
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" id="Search--Streamline-Rounded-Material" height="24" width="24">
  
  <path fill="#000000" d="M9.45 15.775c-1.80265 0 -3.32835 -0.625 -4.577 -1.875C3.624335 12.65 3 11.14165 3 9.375c0 -1.7667 0.625 -3.275 1.875 -4.525025 1.25 -1.25 2.7625 -1.875 4.5375 -1.875 1.775 0 3.28335 0.625 4.525 1.875C15.17915 6.1 15.8 7.60955 15.8 9.37875c0 0.71415 -0.11665 1.40455 -0.35 2.07125 -0.23335 0.66665 -0.58335 1.29165 -1.05 1.875l6.05 6c0.15 0.14265 0.225 0.32415 0.225 0.5445 0 0.2203 -0.075 0.4055 -0.225 0.5555 -0.15 0.15 -0.33515 0.225 -0.5555 0.225s-0.40185 -0.075 -0.5445 -0.225L13.325 14.4c-0.5 0.4333 -1.083 0.7708 -1.749 1.0125 -0.666 0.24165 -1.37465 0.3625 -2.126 0.3625Zm-0.025 -1.5c1.35415 0 2.50525 -0.4792 3.45325 -1.4375C13.8261 11.87915 14.3 10.725 14.3 9.375s-0.4739 -2.5042 -1.42175 -3.4625c-0.948 -0.95836 -2.0991 -1.437525 -3.45325 -1.437525 -1.368 0 -2.53085 0.479165 -3.4885 1.437525C4.978835 6.8708 4.5 8.025 4.5 9.375s0.478835 2.50415 1.4365 3.4625c0.95765 0.9583 2.1205 1.4375 3.4885 1.4375Z" stroke-width="0.5"></path>
</svg> 
{/* 2nd Logo  */}
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" id="Person--Streamline-Rounded-Material" height="24" width="24">
 
  <path fill="#000000" d="M12 11.9751c-1.1 0 -2 -0.35 -2.7 -1.05 -0.7 -0.7 -1.05 -1.6 -1.05 -2.7s0.35 -2 1.05 -2.7c0.7 -0.7 1.6 -1.05 2.7 -1.05s2 0.35 2.7 1.05c0.7 0.7 1.05 1.6 1.05 2.7s-0.35 2 -1.05 2.7c-0.7 0.7 -1.6 1.05 -2.7 1.05Zm-8 6.525v-0.85c0 -0.63335 0.158335 -1.175 0.475 -1.625 0.316665 -0.45 0.725 -0.79165 1.225 -1.025 1.11665 -0.5 2.1875 -0.875 3.2125 -1.125s2.05415 -0.375 3.0875 -0.375 2.05835 0.12915 3.075 0.3875c1.01665 0.25835 2.08265 0.63075 3.198 1.11725 0.52165 0.2355 0.9399 0.5769 1.25475 1.02425 0.31485 0.44735 0.47225 0.98765 0.47225 1.621v0.85c0 0.4125 -0.14685 0.7656 -0.4405 1.05925 -0.29385 0.29385 -0.647 0.44075 -1.0595 0.44075H5.5c-0.4125 0 -0.765585 -0.1469 -1.05925 -0.44075C4.146915 19.2657 4 18.9126 4 18.5001Zm1.5 0h13v-0.85c0 -0.26665 -0.07915 -0.52085 -0.2375 -0.7625 -0.15835 -0.24165 -0.35415 -0.42085 -0.5875 -0.5375 -1.06665 -0.51665 -2.04165 -0.87085 -2.925 -1.0625 -0.88335 -0.19165 -1.8 -0.2875 -2.75 -0.2875s-1.875 0.09585 -2.775 0.2875c-0.9 0.19165 -1.875 0.54585 -2.925 1.0625 -0.23335 0.11665 -0.425 0.29585 -0.575 0.5375 -0.15 0.24165 -0.225 0.49585 -0.225 0.7625v0.85Zm6.5 -8.025c0.65 0 1.1875 -0.2125 1.6125 -0.6375 0.425 -0.425 0.6375 -0.9625 0.6375 -1.6125s-0.2125 -1.1875 -0.6375 -1.6125c-0.425 -0.425 -0.9625 -0.6375 -1.6125 -0.6375s-1.1875 0.2125 -1.6125 0.6375c-0.425 0.425 -0.6375 0.9625 -0.6375 1.6125s0.2125 1.1875 0.6375 1.6125c0.425 0.425 0.9625 0.6375 1.6125 0.6375Z" stroke-width="0.5"></path>
</svg>
{/* 3rd logo  */}
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 24 24"
              id="Favorite--Streamline-Rounded-Material"
              height="24"
              width="24"
            >
              <path
                fill="#000000"
                d="M12 20.4999c-0.17835 0 -0.35985 -0.03225 -0.5445 -0.09675 -0.18465 -0.0645 -0.3471 -0.16765 -0.48725 -0.3095L9.65 18.8749c-2.06665 -1.91665 -3.85835 -3.74585 -5.375 -5.4875C2.758335 11.64575 2 9.8249 2 7.9249c0 -1.5025 0.504165 -2.75735 1.5125 -3.7645 1.008335 -1.007 2.25415 -1.5105 3.7375 -1.5105 0.85 0 1.69165 0.20417 2.525 0.6125 0.83335 0.408335 1.575 1.07917 2.225 2.0125 0.73335 -0.93333 1.49165 -1.604165 2.275 -2.0125 0.78335 -0.40833 1.60835 -0.6125 2.475 -0.6125 1.48335 0 2.72915 0.5035 3.7375 1.5105 1.00835 1.00715 1.5125 2.262 1.5125 3.7645 0 1.9 -0.75835 3.72085 -2.275 5.4625 -1.51665 1.74165 -3.30835 3.57085 -5.375 5.4875l-1.325 1.225c-0.1375 0.13965 -0.29685 0.24125 -0.478 0.30475 -0.18135 0.0635 -0.36365 0.09525 -0.547 0.09525Zm-0.65 -13.575c-0.45 -0.81665 -1.04165 -1.48335 -1.775 -2 -0.73335 -0.516665 -1.50835 -0.775 -2.325 -0.775 -1.1 0 -2 0.35417 -2.7 1.0625C3.85 5.92075 3.5 6.8285 3.5 7.93565c0 0.9595 0.324 1.96965 0.972 3.0305 0.648 1.061 1.423 2.09015 2.325 3.0875 0.902 0.9975 1.83635 1.9254 2.803 2.78375s1.76665 1.57915 2.4 2.1625c0.63335 -0.56665 1.43335 -1.28335 2.4 -2.15 0.96665 -0.86665 1.9 -1.80415 2.8 -2.8125 0.9 -1.00835 1.675 -2.04325 2.325 -3.10475 0.65 -1.0615 0.975 -2.0641 0.975 -3.00775 0 -1.1 -0.35415 -2.00415 -1.0625 -2.7125 -0.70835 -0.70833 -1.60415 -1.0625 -2.6875 -1.0625 -0.83335 0 -1.6125 0.25417 -2.3375 0.7625 -0.725 0.50835 -1.32915 1.17915 -1.8125 2.0125 -0.08335 0.13335 -0.175 0.22915 -0.275 0.2875 -0.1 0.05835 -0.21665 0.0875 -0.35 0.0875 -0.13335 0 -0.25415 -0.02915 -0.3625 -0.0875s-0.19585 -0.15415 -0.2625 -0.2875Z"
                stroke-width="0.5"
              ></path>
            </svg>
            {/* 4th logo  */}
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" id="Local-Mall--Streamline-Rounded-Material" height="24" width="24">

  <path fill="#000000" d="M4.5 22c-0.4125 0 -0.765585 -0.1469 -1.05925 -0.44075C3.146915 21.2656 3 20.9125 3 20.5V7.25c0 -0.4125 0.146915 -0.76565 0.44075 -1.0595C3.734415 5.89685 4.0875 5.75 4.5 5.75h2.75c0 -1.3 0.44585 -2.416665 1.3375 -3.35C9.47915 1.466665 10.575 1 11.875 1c1.34865 0 2.49835 0.458335 3.449 1.375C16.27465 3.291665 16.75 4.416665 16.75 5.75H19.5c0.4125 0 0.76565 0.14685 1.0595 0.4405 0.29365 0.29385 0.4405 0.647 0.4405 1.0595V20.5c0 0.4125 -0.14685 0.7656 -0.4405 1.05925C20.26565 21.8531 19.9125 22 19.5 22H4.5Zm0 -1.5h15V7.25H4.5V20.5Zm4.25 -14.75h6.5c0 -0.916665 -0.3125 -1.6875 -0.9375 -2.3125S12.91665 2.5 12 2.5s-1.6875 0.3125 -2.3125 0.9375S8.75 4.833335 8.75 5.75Zm3.243 7.5c1.22885 0 2.34135 -0.42915 3.3375 -1.2875 0.99635 -0.85835 1.4945 -1.82085 1.4945 -2.8875 0 -0.18335 -0.075 -0.34585 -0.225 -0.4875 -0.15 -0.14165 -0.3346 -0.2125 -0.55375 -0.2125 -0.1805 0 -0.34165 0.0625 -0.4835 0.1875 -0.14185 0.125 -0.2294 0.2875 -0.26275 0.4875 -0.16665 0.78335 -0.5625 1.42915 -1.1875 1.9375C13.4875 11.49585 12.78335 11.75 12 11.75c-0.78335 0 -1.4875 -0.25415 -2.1125 -0.7625 -0.625 -0.50835 -1.02085 -1.15415 -1.1875 -1.9375 -0.03335 -0.2 -0.125 -0.3625 -0.275 -0.4875 -0.15 -0.125 -0.31665 -0.1875 -0.5 -0.1875 -0.2 0 -0.375 0.07085 -0.525 0.2125 -0.15 0.14165 -0.225 0.30415 -0.225 0.4875 0 1.06785 0.49585 2.0306 1.4875 2.88825 0.99165 0.85785 2.10185 1.28675 3.3305 1.28675Z" stroke-width="0.5"></path>
</svg>
            
          </div>
          
           
          
        </div>
      </nav>
    </div>
  );
}

export default Navbar;
